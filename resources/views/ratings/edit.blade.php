    ...
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('ratings.update', $rating) }}">
            ...
            <input type='text' id="artist" name="artist" value="{{ old('artist', $rating->artist) }}"
                   class="block w-full border-gray-300 dark:bg-gray-800 dark:text-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
            ...
            <input type='number' id="stars" name="stars" value="{{ old('stars', $rating->stars) }}"
                   class="block w-20 border-gray-300 dark:bg-gray-800 dark:text-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
            <p class="text-lg text-gray-800 dark:text-gray-200">/ 5</p>
            <x-input-error :messages="$errors->get('stars')" class="mt-2"/>
            <div class="mt-4 space-x-2">
                <x-primary-button>{{ __('Save') }}</x-primary-button>
                <a class="text-gray-800 dark:text-gray-300" href="{{ route('ratings.index') }}">{{ __('Cancel') }}</a>
            </div>
        </form>
    </div>
</x-app-layout>
